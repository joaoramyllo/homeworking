...
class StudentInterestsView(UpdateView):
    ...
    success_url = reverse_lazy("classroom:student_profile")
...
class QuizListView(SearchListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Quiz.objects.filter(subject__in=student_interests)
...
class TakenQuizListView(ListView):
    ...
    def get_queryset(self):
        queryset = self.request.user.student.taken_quizzes.select_related("quiz", "quiz__subject").order_by("quiz__name")
        return queryset
...
def take_quiz(request, pk):
            ...
            with transaction.atomic():
                ...
                if student.get_unanswered_questions(quiz).exists():
                    ...
                    if score < 7.0:
                        messages.warning(
                            request,
                            "Mais sorte da próxima vez! Sua pontuação no questionário %s foi de %s." % (quiz.name, score),
                        )

                    else:
                        messages.success(
                            request,
                            "Parabéns! Você concluiu o avaliação %s com sucesso! Você marcou %s pontos." % (quiz.name, score),
                        )
    ...
    return render(
        request,
        "classroom/students/take_quiz_form.html",
        {
            "quiz": quiz,
            "question": question,
            "form": form,
