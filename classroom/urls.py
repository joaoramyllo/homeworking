...
urlpatterns = [
    ...
    path("alunos/responder", students.QuizListView.as_view(), name="quiz_list"),
    ...
    path(
        "professor/avaliacao/<int:quiz_pk>/cadastrar/<int:question_pk>/deletar/",
...
urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)  # to use media files
urlpatterns += static(settings.STATIC_URL, document_root=settings.STATIC_ROOT)  # to use static files
